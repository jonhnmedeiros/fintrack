...
export function ExpenseByCategoryChart({ transactions, categories, isLoading }: ExpenseByCategoryChartProps) {
  ...
  transactions
    ...
    .forEach((tx) => {
      ...
      const parent = cat.parentId ? categoryById.get(cat.parentId) : cat
      const parentName = parent?.name || cat.name
